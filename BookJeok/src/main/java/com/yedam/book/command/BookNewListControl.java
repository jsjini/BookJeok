...
//신간도서
public class BookNewListControl implements Control {

	@Override
	public void execute(HttpServletRequest req, HttpServletResponse resp) {
		BookService svc = new BookServiceImpl();
		List<BookVO> list = svc.bookNewList();
		
		req.setAttribute("bookNewList", list);
		
		// 페이지를 이동(forward)
		// 사용자가 URL을 입력시 "/WEB-INF/book/bookList.jsp"페이지 재이동
		RequestDispatcher rd = req.getRequestDispatcher("book/bookNewList.tiles");
		try {
